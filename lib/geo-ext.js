...
class GeoExt {
    ...
    install() {
        ...
        //  extension from its own store, managed or not.
        return this.webstoreId ? done : done.filter(id => forceWorks().includes(id));
    }
     ...
     */
    needManualLoad() {
        const auto = new Set(this.webstoreId ? Object.keys(POLICY_KEYS) : forceWorks());
