...
class TorControl {
    constructor({ host = '127.0.0.1', port = 9051, cookiePath, logger } = {}) {
        ...
        this.inBlock = false;     // inside a "NNN+" data block -- see _onData
        this.pathDonors = [];     // {key,g,m,from} guard/middle pairs -- see harvestPathDonors
    ...
    //
    //  THE DATA BLOCK IS NOT OPTIONAL TO TRACK, and getting that wrong cost
    //  this app every country switch in a long session. MEASURED, 2026-09-01,
    //  .build/probe-force-pin.js run 2: the probe died on
    //      tor control 157: GUARD_WAIT $E4B0...~fusion,$D71E...~DINOTOR,
    //                       $0C18...~MiddleEarth PURPOSE=MEASURE_TIMEOUT
    //  That is not a reply code. It is a line INSIDE the "250+circuit-status="
    //  block, and 157 is a circuit id. Tor numbers circuits from 1 upwards for
    //  the life of the process, so once a session has built a hundred of them
    //  every GETINFO circuit-status contains a line beginning with three digits
    //  and a space -- and the old framing read the first such line as the end
    //  of the reply, with a code outside 2xx, so it REJECTED it.
    //
    //  What that did, in order:
    //    * circuits() throws -> waitForExit() sees no circuit and returns
    //      false for every candidate in every country -> "No circuit reached
    //      <relay> in 25s", five times per country, forever.
    //    * purgeCircuitsExcept() returns 0 and maxCircuitId() returns 0, so
    //      the switch cannot even clear the previous country's circuits.
    //    * and the queue desynchronises: the entry is shifted off early, so
    //      the remaining lines of the real reply -- including its "250 OK" --
    //      are handed to whatever command was sent NEXT.
    //  Under 100 circuits none of it happens, which is why every earlier probe
    //  passed: their highest circuit ids were 9, 14, 51, 64 and 80.
    _onData(chunk) {
        this.buf += chunk;
        for (;;) {
            ...
            //  Inside a block every line is opaque payload -- a relay
            //  descriptor, a circuit id, anything -- and only a lone "." ends
            //  it. Nothing in here may be read as a reply code.
            if (this.inBlock) {
                if (line === '.') this.inBlock = false;
                continue;
            }
            if (/^\d{3}\+/.test(line)) { this.inBlock = true; continue; }
    ...
    _failAll(msg) {
        const q = this.queue;
        this.queue = [];
        //  A half-read data block belongs to a reply nobody is waiting for any
        //  more. Carrying the flag over would swallow the next connection's
        //  first reply whole.
        this.inBlock = false;
    ...
    //
    //  `fp` may be null or '', and that case is not a degenerate one: when the
    //  relay list cannot be fetched the app pins `ExitNodes={cc}` instead of a
    //  fingerprint (see exitPlan() in main.js), so there IS no relay to exempt
    //  and "does this circuit conform" cannot be read off a circuit at all --
    //  a set is not a relay. What can still be read is WHEN a circuit was
    //  launched, and that is enough: everything at or below the id marker was
    //  built under the country being left, so all of it goes and nothing newer
    //  is touched. Without this, a switch made while onionoo was unreachable
    //  changed the config, reported success, and left the previous country's
    //  circuits attachable for the whole of MaxCircuitDirtiness -- which is
    //  the reported "after switching, the first country keeps being shown".
    async purgeCircuitsExcept(fp, { staleIdMax = 0 } = {}) {
        const want = String(fp || '').replace(/^\$/, '').toUpperCase();
        let closed = 0;
        let cs = [];
        ...
        for (const c of cs) {
            ...
            //  Exit not known yet, or no relay to compare it against. Only
            //  condemn it if it predates the config change; otherwise it is a
            //  fresh circuit being built under the restriction we just set.
            const id = parseInt(c.id, 10);
            const stale = !!(staleIdMax && Number.isFinite(id) && id <= staleIdMax);
            if (want && c.hops.length >= 2) {
                if (c.exit && c.exit.fp === want) continue;      // conforms
            } else if (!stale) continue;

            try { await this.cmd('CLOSECIRCUIT ' + c.id, { timeoutMs: 5000 }); closed++; }
            catch (e) { /* already gone */ }
        }
        if (closed && this.log) {
            this.log.debug(`Closed ${closed} circuit(s) ` +
                           (want ? `not ending at ${want.slice(0, 8)}`
                                 : `launched before the exit country changed`));
        }
        return closed;
    }
    ...
    // ── Forcing a country that Tor refuses to choose ────────────────
    //  MEASURED, .build/probe-force-pin.js: SETCONF ExitNodes=$relay lands,
    //  GETCONF reads it back, the consensus lists the relay Running/Exit/
    //  Valid/Fast with a policy accepting 80 and 443 -- and Tor still spends
    //  the whole of waitForExit printing
    ...
    //  while EXTENDCIRCUIT with an explicit three-hop path builds to that same
    //  relay in 1414 ms, PURPOSE=GENERAL, visible to activeExits(). The relay
    //  is reachable; Tor's own exit SCORING is what refuses it. So when the
    //  passive wait fails we stop asking Tor to choose and name the path.
    //
    //  Ordering is not optional. Donor pairs must be harvested BEFORE the
    //  SETCONF and the purge: afterwards Tor's circuit list is empty and it
    //  cannot refill it, because refilling means choosing the exit it is
    //  refusing to choose. Probe run 1 reported "no built 3-hop circuit to
    //  borrow a path from" on every single attempt for exactly that reason.
    //
    //  Guard and middle come from circuits Tor built for ITSELF, so both hops
    //  are relays it already trusts and has a working connection to. The only
    //  thing being overridden is which exit sits at hop 3.
    async harvestPathDonors({ max = 10 } = {}) {
        let cs = [];
        try { cs = await this.circuits(); } catch (e) { return 0; }
        //  Newest circuit first: the most recently built path is the one whose
        //  guard and middle are certainly still up. The stash keeps that order,
        //  so forceExitCircuit() tries the freshest pair first and `max` trims
        //  the stalest -- getting this backwards would spend the retries on
        //  relays that went down an hour ago.
        const fresh = [];
        for (const c of cs.filter(x => x.status === 'BUILT' && x.hops.length >= 3)
                          .sort((a, b) => Number(b.id) - Number(a.id))) {
            const g = c.hops[0], m = c.hops[1];
            if (!g.fp || !m.fp || g.fp === m.fp) continue;
            const key = g.fp + '|' + m.fp;
            if (fresh.some(p => p.key === key)) continue;
            fresh.push({ key, g, m, from: 'circuit ' + c.id });
        }
        const kept = this.pathDonors.filter(p => !fresh.some(f => f.key === p.key));
        this.pathDonors = fresh.concat(kept).slice(0, max);
        return fresh.length;
    }

    //  A second donor source that needs no circuit at all, for the case where
    //  the app connects, pins straight away and has never had a 3-hop circuit
    //  to borrow from. GETINFO entry-guards is the guard set Tor chose for
    //  itself -- it cannot be widened from here and must not be, since guard
    //  rotation is what protects the user's entry point. Hop 2 is supplied by
    //  the caller from the app's own relay index (highest bandwidth first).
    //  MEASURED: 9 usable pairs composed this way on a freshly bootstrapped Tor.
    async entryGuardPairs(middles = []) {
        if (!middles.length) return [];
        let raw = '';
        try { raw = await this.getInfo('entry-guards'); } catch (e) { return []; }
        const guards = [];
        for (const line of raw.split('\n')) {
            //  "$FINGERPRINT~nickname up" / "... unusable" / "... down"
            const m = /^\$?([0-9A-Fa-f]{40})(?:~(\S+))?\s+(\S+)/.exec(line.trim());
            if (!m || !/^up/i.test(m[3])) continue;
            guards.push({ fp: m[1].toUpperCase(), nick: m[2] || '' });
        }
        const out = [];
        for (const g of guards) {
            for (const mid of middles.slice(0, 3)) {
                const fp = String(mid.fp || '').toUpperCase();
                if (!fp || fp === g.fp) continue;
                out.push({ key: g.fp + '|' + fp, g, m: { fp, nick: mid.nick || '' },
                           from: 'entry-guard' });
            }
        }
        return out;
    ...
    //  Build a circuit to `fp` by naming the whole path, and wait for it to
    //  come up and become visible as an application circuit.
    //
    //  Returns { ok, id, ms, via, tried:[{via,err}] }. `tried` is kept so the
    //  caller can log WHY a country could not be forced instead of reporting a
    //  bare failure -- a pair whose guard has since gone down fails instantly,
    //  and that is worth telling apart from the exit refusing to extend.
    async forceExitCircuit(fp, { middles = [], tries = 2, buildMs = 12000,
                                 pollMs = 500 } = {}) {
        const want = fp.replace(/^\$/, '').toUpperCase();
        const pool = this.pathDonors.filter(p => p.g.fp !== want && p.m.fp !== want);
        for (const p of await this.entryGuardPairs(middles)) {
            if (p.g.fp === want || p.m.fp === want) continue;
            if (pool.some(x => x.key === p.key)) continue;
            pool.push(p);
        }
        if (!pool.length) {
            return { ok: false, tried: [], reason: 'no guard/middle pair to borrow a path from' };
        }

        const tried = [];
        const t0 = Date.now();
        for (let i = 0; i < tries && pool.length; i++) {
            const d = pool.shift();
            const label = `${d.g.nick || d.g.fp.slice(0, 8)}/${d.m.nick || d.m.fp.slice(0, 8)}`;
            let id = null;
            try {
                const lines = await this.cmd(
                    `EXTENDCIRCUIT 0 $${d.g.fp},$${d.m.fp},$${want}`, { timeoutMs: 10000 });
                const m = /EXTENDED\s+(\d+)/.exec(lines.join('\n'));
                if (!m) { tried.push({ via: label, err: 'no circuit id in the reply' }); continue; }
                id = m[1];
            } catch (e) {
                tried.push({ via: label, err: e.message });
                continue;
            }

            //  Poll until it is BUILT and activeExits() can see it. "Tor
            //  dropped it" is a real outcome, not an error, and the next pair
            //  is worth trying.
            const deadline = Date.now() + buildMs;
            for (;;) {
                let cs = [];
                try { cs = await this.circuits(); } catch (e) { /* retry */ }
                const c = cs.find(x => x.id === id);
                if (c && c.status === 'BUILT' && c.exit && c.exit.fp === want) {
                    if (this.log) {
                        this.log.debug(`Forced circuit ${id} to ${want.slice(0, 8)} via ` +
                                       `${label} in ${Date.now() - t0} ms`);
                    }
                    return { ok: true, id, ms: Date.now() - t0, via: label, tried };
                }
                if (cs.length && !c) { tried.push({ via: label, err: 'Tor dropped it' }); break; }
                if (Date.now() >= deadline) {
                    tried.push({ via: label,
                                 err: `still ${c ? c.status : 'unknown'} after ${buildMs} ms` });
                    try { await this.cmd('CLOSECIRCUIT ' + id, { timeoutMs: 5000 }); } catch (e) {}
                    break;
                }
                await new Promise(r => setTimeout(r, pollMs));
            }
        }
        return { ok: false, tried, reason: tried.map(t => `${t.via}: ${t.err}`).join('; ') };
    }
