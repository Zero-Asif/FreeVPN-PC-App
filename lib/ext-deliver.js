...
// ════════════════════════════════════════════════════════════════════
//  The early bind: the port up before Electron is
// ════════════════════════════════════════════════════════════════════
//  WHY THIS EXISTS, and it is four measured timestamps rather than a hunch.
//
//  MEASURED 2026-09-01, on the restart the app asks for after an install
//  (schtasks last-run time plus this app's own log):
//
//     21:37:17   logon                     (task start minus the trigger delay)
//     21:37:27   the task's action starts   <-- <Delay>PT10S</Delay>, now removed
//     21:38:09.5 app.whenReady() fires      <-- 42.5 s of Electron cold start
//     21:38:38.4 the port is finally bound  <-- 28.9 s of profile scanning
//
//  Eighty-one seconds after logon. The user's desktop is usable long before
//  that, and a browser started in the meantime reads its external-extensions
//  provider, finds nothing listening on the port every policy names, installs
//  nothing, and does not look again until its NEXT start. That is exactly the
//  report: "restart er por Edge e dhuke dekhi kichu asenai, browser kete diye
//  abar open korlam tokhon asche."
//
//  The trigger delay and the profile scan are gone (see deliverTaskXml() and
//  serveUntilDelivered()), but 42.5 s of Chromium browser-process init would
//  still be in front of the port, and no amount of reordering inside
//  app.whenReady() can touch time spent BEFORE it resolves.
//
//  So the bind moves out of it. Serving these two files needs `http`, `fs` and
//  the bundle on disk -- no window, no GPU, no Electron API at all -- and the
//  Electron main process runs its entry script, with a working libuv loop, well
//  before whenReady resolves. main.js calls this at module scope for --fp-deliver
//  and the listener is up within seconds of process start instead of after
//  Chromium has finished booting.
//
//  Two details that are not decoration:
//    * the log does not exist yet (Logger.init runs inside whenReady), so lines
//      are buffered and replayed into the real log by serveUntilDelivered. The
//      timestamps in the file then still show when the port actually bound.
//    * a failed bind is not an error here. The boot pass or an elevated app
//      start may hold the port; serveUntilDelivered's existing grace loop is
//      what handles that, and it is reached with port === null exactly as before.
let _early = null;

/**
 * Bind the delivery port now, before anything else in the process is ready.
 *
 * Idempotent, never throws, and never blocks: it returns immediately with a
 * handle whose `.promise` resolves to the port or to null.
 *
 * @returns {{d: Deliverer, at: number, port: number|null, promise: Promise<number|null>}}
 */
function serveEarly({ stateDir }) {
    if (_early) return _early;
    const buffered = [];
    const log = {};
    for (const level of ['debug', 'info', 'warn', 'error', 'success'])
        log[level] = (msg, meta) => buffered.push([level, msg, meta]);

    const d  = new Deliverer({ stateDir, log });
    const at = Date.now();
    _early = { d, buffered, at, port: null, promise: null };
    try {
        _early.promise = d.start({ quiet: true }).then(port => {
            _early.port = port;
            if (port) buffered.push(['debug', 'The delivery port was bound at module scope, ' +
                `${(process.uptime()).toFixed(1)}s into this process and before ` +
                'app.whenReady() -- a browser started now is served rather than told nothing']);
            return port;
        }, () => null);
    } catch (e) { _early.promise = Promise.resolve(null); }
    return _early;
}

/** The handle serveEarly() made, or null if it was never called. Test hook. */
function earlyServer() { return _early; }
function _resetEarly() { if (_early) { try { _early.d.stop(); } catch (e) {} } _early = null; }
...
async function serveUntilDelivered({ stateDir, log, capMs = CAP_MS, pollMs = POLL_MS }) {
    ...
    let d, t0;
    if (early) {
        //  Adopt the listener bound at module scope. `t0` is when THAT started,
        //  not when this function was reached, so every "(Ns)" in the log below
        //  is measured from the same origin it always was.
        d  = early.d;
        t0 = early.at;
        await early.promise;
        //  Replay what could not be logged yet: Logger.init only ran inside
        //  app.whenReady(), and the bind happened before it.
        for (const [lvl, msg, meta] of early.buffered) {
            const fn = log[lvl] || log.info;
            try { meta === undefined ? fn.call(log, msg) : fn.call(log, msg, meta); } catch (e) {}
        }
        early.buffered.length = 0;
    } else {
        d  = new Deliverer({ stateDir, log });
        t0 = Date.now();
    }
    d.log = log;
    const sleep = ms => new Promise(r => setTimeout(r, ms));

    //  ── The port first, the profile scan second ────────────────────
    //  missing() calls browsers.detectChromium() and then reads every profile of
    //  every installed browser -- registry queries and JSON files. Measured on
    //  2026-09-01, on the restart the app asks for: this task was recognised at
    //  21:38:09.7 and did not answer 8081 until 21:38:38.4. Twenty-nine seconds,
    //  all of it spent deciding WHEN TO STOP serving, before anything was being
    //  served at all -- and a browser opened inside that window reads its
    //  external-extensions provider, finds a dead port, and offers the user
    //  nothing until its next start. That is the reported "extension enable
    //  option ta sathe sathe asche na".
    //
    //  Binding first costs nothing: the scan's only job is the exit condition,
    //  and if it turns out every browser already has the extension the listener
    //  is closed again in the next few lines without a single fetch served.
    let port = d.port || await d.start({ quiet: true });
...
function deliverTaskXml(exePath) {
    return '﻿' + [
        ...
        '  </RegistrationInfo>',
        //  No <Delay>. It used to be PT10S, out of politeness at logon, and that
        //  politeness was the bug: a browser reads its external-extensions
        //  provider once, while it starts, and a browser opened before this
        //  helper answers 8081 is told nothing and offers nothing until its NEXT
        //  start. Measured on 2026-09-01 after the restart the app asks for --
        //  task launched 21:37:27, the port did not answer until 21:38:38 -- and
        //  reported as "restart sese edge e dhuke dekhi kono extension install er
        //  jonno kichu asenai, then browser kete diye abar open korlam tokhon
        //  asche". Ten of those seventy-one seconds were this line.
        ...
        //  Priority 4 (NORMAL_PRIORITY_CLASS), not the 7 this used to carry.
        //  7 is below-normal with throttled I/O, and this task's whole job is to
        //  answer a port before the user's first browser start -- at logon it was
        //  competing for the disk against everything else Windows starts, and
        //  losing. Nothing here runs long enough for normal priority to cost the
        //  user anything: it opens a socket and serves two small files.
        '    <ExecutionTimeLimit>PT4H</ExecutionTimeLimit><Priority>4</Priority>',
        '  </Settings>',
...
module.exports = { DELIVER_TASK, MANIFEST_PATH, CRX_PATH, CAP_MS, POLL_MS,
                   ...
                   serveEarly, earlyServer, _resetEarly,
