...
async function taskSetup(log, ctx) {
    ...
    //  entry names: every fork unpacks the bytes at location 6 and holds them at
    //  disable_reasons 8192 (EXTERNAL_EXTENSION) until the user accepts. An
    //  earlier round recorded Edge at location 7 here; that was its POLICY route
    //  winning the cold-start race, and on 2026-08-31 this one won instead --
    //  Edge arrived switched off and the user had to enable it by hand. So a
    //  browser whose forcelist is measured to work is not offered this route --
    //  but only on a device an organization manages, because an unmanaged Edge
    //  answers an off-store force-install with [BLOCKED] and installs nothing
    //  (measured 2026-09-01 in edge://policy; see geo-ext.js forceWorks()). On a
    //  personal PC, therefore, this list holds every fork on the machine, Edge
    //  included: switched-off is better than absent, and absent is what skipping
    ...
    if (ext3.length) {
        log.info(`External-extensions entry written for ${ext3.join(', ')} -- each of those ` +
                 ...
                 'it then keeps the extension switched off until the user accepts it once');
    }
