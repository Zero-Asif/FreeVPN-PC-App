...
//  extensions/browser/disable_reason.h. The VALUES are permanent -- that header
//  forbids removing or reordering them because they feed histograms, so retired
//  reasons leave their bit vacant for ever and this table cannot drift with a
//  Chromium release. Read back from the header on 2026-09-02 (googlesource and
//  the github mirror, independently) after the two labels below were found wrong:
//
//      262144  was labelled NOT_ALLOWLISTED here. It is 1<<18, retired --
//              DISABLE_REMOTELY_FOR_MALWARE. NOT_ALLOWLISTED is 1<<20.
//      524288  was labelled UNSUPPORTED_MANIFEST_VERSION here. It is 1<<19,
//              REINSTALL. UNSUPPORTED_MANIFEST_VERSION is 1<<23.
//
//  Those two mattered: both wrong labels sat on bits Chromium can really set for
//  an off-store MV3 extension, so the coverage report would have named the wrong
//  cause and the user would have been sent to fix the wrong thing.
//
//  Grouped by who switched it off, because that is the only question worth
//  answering when the extension goes dark:
//    the user            USER_ACTION
//    a prompt awaiting the user   EXTERNAL_EXTENSION, PERMISSIONS_INCREASE,
//                                 CUSTODIAN_APPROVAL_REQUIRED
//    the browser, by itself       NOT_VERIFIED, GREYLIST, CORRUPTED,
//                                 NOT_ALLOWLISTED, UNSUPPORTED_MANIFEST_VERSION,
//                                 UNSUPPORTED_DEVELOPER_EXTENSION,
//                                 SIDELOAD_WIPEOUT
//    an administrator     BLOCKED_BY_POLICY, UPDATE_REQUIRED_BY_POLICY,
...
const DISABLE_REASON = {
    ...
    524288: 'REINSTALL', 1048576: 'NOT_ALLOWLISTED',
    2097152: 'NOT_ASH_KEEPLISTED',
    4194304: 'PUBLISHED_IN_STORE_REQUIRED_BY_POLICY',
    8388608: 'UNSUPPORTED_MANIFEST_VERSION',
    16777216: 'UNSUPPORTED_DEVELOPER_EXTENSION',
    33554432: 'UNKNOWN',
    67108864: 'BLOCKED_BY_CLOUD_POLICY_CHECK',
};
//  A bit this table does not name still counts. `enabled` is decided by
//  bits === 0 (below), never by whether a label was found, so a Chromium newer
//  than this table reads as disabled-for-an-unnamed-reason rather than enabled.
const DISABLE_REASON_NAMED_MASK = Object.keys(DISABLE_REASON)
    .reduce((a, k) => a | Number(k), 0);

/**
 * Bitmask -> reason names, with anything unnamed reported as a bit rather than
 * dropped. A list that silently loses a bit reads as "disabled for no reason",
 * which is the one answer that cannot be true.
 */
function namesFor(bits) {
    const out = Object.keys(DISABLE_REASON)
        .filter(k => bits & Number(k)).map(k => DISABLE_REASON[k]);
    const rest = bits & ~DISABLE_REASON_NAMED_MASK;
    if (rest) out.push(`UNNAMED_BIT_${rest}`);
    return out;
}

//  Who switched it off. Ask 2 -- "the browser must not be able to disable this
//  behind the user's back" -- is answered per reason, and the answers are not
//  alike: USER_ACTION is the user exercising the control they are supposed to
//  have and must never be undone; EXTERNAL_EXTENSION is a prompt waiting for
//  them; the browser's own enforcement is the only group a policy exemption can
//  pre-empt; and an administrator's decision is not ours to overrule.
const DISABLE_AUTHOR = {
    USER_ACTION: 'user',
    EXTERNAL_EXTENSION: 'prompt',
    PERMISSIONS_INCREASE: 'prompt',
    CUSTODIAN_APPROVAL_REQUIRED: 'prompt',
    REMOTE_INSTALL: 'prompt',
    NOT_VERIFIED: 'browser',
    GREYLIST: 'browser',
    CORRUPTED: 'browser',
    NOT_ALLOWLISTED: 'browser',
    SIDELOAD_WIPEOUT: 'browser',
    UNSUPPORTED_MANIFEST_VERSION: 'browser',
    UNSUPPORTED_DEVELOPER_EXTENSION: 'browser',
    UNSUPPORTED_REQUIREMENT: 'browser',
    REINSTALL: 'browser',
    RELOAD: 'browser',
    BLOCKED_BY_POLICY: 'admin',
    UPDATE_REQUIRED_BY_POLICY: 'admin',
    PUBLISHED_IN_STORE_REQUIRED_BY_POLICY: 'admin',
    BLOCKED_BY_CLOUD_POLICY_CHECK: 'admin',
};

/**
 * The strongest author present, worst-first. A row disabled by BOTH the user and
 * the browser must report the user, because re-enabling it would override a
 * choice we promised to leave alone. An unnamed bit outranks 'browser' and
 * 'prompt' so a reason this table does not know is never reported as merely
 * waiting for a click.
 */
function disableAuthor(names) {
    const order = ['user', 'admin', 'unknown', 'browser', 'prompt'];
    let best = null;
    for (const n of names || []) {
        const a = DISABLE_AUTHOR[n] || 'unknown';
        if (best === null || order.indexOf(a) < order.indexOf(best)) best = a;
    }
    return best;
}
...
module.exports = { ALL, CHROMIUM, GECKO, WININET, expand, firstExisting,
                   ...
                   namesFor, disableAuthor, DISABLE_AUTHOR,
